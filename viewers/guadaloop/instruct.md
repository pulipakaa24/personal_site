...
Next, the IMU's should be rotated to face almost to the front but slightly to the right (the normal from the IMUs should be pointing toward the right of the camera, so we get a natural look. Then, for the storyboard elements, when you say "two halves, two IMUs," I need to completely revamp this. First off, instead of just dragging one to the side, I need you to make the thigh hinge (upper leg) rotate towards the back and then up, almost so that it's straight on but inverted but not quite (around 160 degrees). The current hinge-rotation animation also has an issue where as soon as the rotation begins, the IMU on the moving part (lowerleg currently but upper leg after this edit) immediately gets blown off into oblivion. Not sure why that's happening. Once these changes are made, the storyboard should stay here and just bring in the leg visualization objects before starting another small back-and-forth hinge animation with them, similar to what's currently there. The leg silhouettes don't currently align through the circular rings throughout the animation once they appear, though. You can probably see that as the brace moves one way, the leg movement is in a random arbitrary direction. My guess is that you're rotating the wrong leg object, or that the leg objects are wrongly oriented. It seems like the one moving is lagging by 90 degrees, and the one stationary is ahead by 90 degrees. I'm sure you can see this. ensure the cylindrical bodies you made go through their respective openings. 
